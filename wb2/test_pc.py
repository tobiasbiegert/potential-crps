...
def main():
    ...
    # GraphCast vs FuXi (ERA5 ground truth)
    with ProgressBar():
        graphcast_fuxi_vs_era5_p = block_permutation_test_dataset(
            graphcast_vs_era5_crps.sel(time=fuxi_vs_era5_crps.time),
            fuxi_vs_era5_crps, 
            variables=variables[:-1]
            ).compute()
    graphcast_fuxi_vs_era5_p.to_netcdf('results/graphcast_fuxi_vs_era5_p.nc')
    
    # GraphCast vs FuXi (IFS‑Analysis ground truth)
    with ProgressBar():
        graphcast_fuxi_vs_ifs_analysis_p = block_permutation_test_dataset(
            graphcast_vs_ifs_analysis_crps.sel(time=fuxi_vs_ifs_analysis_crps.time),
            fuxi_vs_ifs_analysis_crps,
            variables=variables[:-1]
            ).compute()
    graphcast_fuxi_vs_ifs_analysis_p.to_netcdf('results/graphcast_fuxi_vs_ifs_analysis_p.nc')
    
    # Pangu vs FuXi (ERA5 ground truth)
    with ProgressBar():
        pangu_fuxi_vs_era5_p = block_permutation_test_dataset(
            pangu_vs_era5_crps.sel(time=fuxi_vs_era5_crps.time),
            fuxi_vs_era5_crps,
            variables=variables[:-1]
            ).compute()
    pangu_fuxi_vs_era5_p.to_netcdf('results/pangu_fuxi_vs_era5_p.nc')
    
    # Pangu vs FuXi (IFS‑Analysis ground truth)
    with ProgressBar():
        pangu_fuxi_vs_ifs_analysis_p = block_permutation_test_dataset(
            pangu_vs_ifs_analysis_crps.sel(time=fuxi_vs_ifs_analysis_crps.time),
            fuxi_vs_ifs_analysis_crps,
            variables=variables[:-1]
            ).compute()
    pangu_fuxi_vs_ifs_analysis_p.to_netcdf('results/pangu_fuxi_vs_ifs_analysis_p.nc')
    
    # IFS‑HRES vs FuXi (ERA5 ground truth)
    with ProgressBar():
        ifs_hres_fuxi_vs_era5_p = block_permutation_test_dataset(
            ifs_hres_vs_era5_crps.sel(time=fuxi_vs_era5_crps.time),
            fuxi_vs_era5_crps,
            variables=variables[:-1]
            ).compute()
    ifs_hres_fuxi_vs_era5_p.to_netcdf('results/ifs_hres_fuxi_vs_era5_p.nc')
    
    # IFS‑HRES vs FuXi (IFS‑Analysis ground truth)
    with ProgressBar():
        ifs_hres_fuxi_vs_ifs_analysis_p = block_permutation_test_dataset(
            ifs_hres_vs_ifs_analysis_crps.sel(time=fuxi_vs_ifs_analysis_crps.time),
            fuxi_vs_ifs_analysis_crps,
            variables=variables[:-1]
            ).compute()
    ifs_hres_fuxi_vs_ifs_analysis_p.to_netcdf('results/ifs_hres_fuxi_vs_ifs_analysis_p.nc')

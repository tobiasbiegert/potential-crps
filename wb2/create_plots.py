...
def bar_diff(ax, diffs_by_var, title, ylabel=None):
    x = np.arange(len(lead_time_days))
    width = 0.22

    ax.axhline(0, color='gray', linewidth=0.4)

    for i, (var, lab) in enumerate(zip(variables[:-1], titles[:-1])):
        d = diffs_by_var[var].values
        ax.bar(x + (i - 1)*width, d, width=width, label=lab, color=var_colors[var], edgecolor='black', linewidth=0.5)

    
    ax.set_xticks(x)
    ax.set_xticklabels([int(d) for d in lead_time_days])
...
# Helper function for latitude curve metrics plot
def style_axis(ax, ylabel, title, x_ticks, x_ticklabels):
...
okabe_ito_norm = {name: tuple(v/255 for v in rgb) for name, rgb in okabe_ito.items()}

oi_cmap = {
    'HRES': okabe_ito_norm['sky_blue'],
    'PW': okabe_ito_norm['orange'],
    'PW-ERA5': okabe_ito_norm['orange'],
    'GC': okabe_ito_norm['bluish_green'],
    'GC-ERA5': okabe_ito_norm['bluish_green'],
    'ERA5_PC0': okabe_ito_norm['vermilion'],
    'IFS_PC0': okabe_ito_norm['reddish_purple'],
    'ERA5 Climatology': okabe_ito_norm['black'],
    'Persistence':  okabe_ito_norm['vermilion'],
}

var_colors = {
    'mean_sea_level_pressure': okabe_ito_norm['sky_blue'], 
    '2m_temperature': okabe_ito_norm['orange'], 
    '10m_wind_speed': okabe_ito_norm['bluish_green'],
    'total_precipitation_24hr': okabe_ito_norm['vermilion']
}
...
units = [r'$\left[ \text{Pa} \right]$', r'$ \left[ \text{K} \right]$', r'$ \left[ \text{ms}^{-1} \right] $', r'$\left[ \text{md}^{-1} \right]$']
...
def main():
    ...
    pangu_ifs_hres_vs_era5_p = xr.open_dataset('results/pangu_ifs_hres_vs_era5_p.nc', decode_timedelta=True).load()
    ...
    lat_ticklabels = ['90°S','60°S','30°S','0°','30°N','60°N','90°N']   
    
    # ---------- plot ----------
    fig, axes = plt.subplots(2, 3, figsize=(16, 10))
    ax_rmse, ax_pc, _ = axes[0]
    ax_acc,  ax_pcs, ax_cpa = axes[1]
    ...
    for ax, var, title, xlim_val, ylim_val, unit in zip(axes, variables[:-1], titles[:-1], xlims, ylims, units[:-1]):
        ...
        ax.set_ylabel('ENS CRPS ' + unit, fontsize=20)
